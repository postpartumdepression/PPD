...
ppd = ppd[(ppd['diffppd'] > 7 ) & (ppd['diffppd'] < 367)]
...
antidepressant3 = antidepressant3[(antidepressant3['diffanti'] > 7 ) & (antidepressant3['diffanti'] < 367)]
...
#################################
#exclude ppd diagnose 0-7days after childbirth
ppdex = pd.read_csv(folder+ "ppd0820.csv", delimiter=',', header=0)
ppdex[['person_id']].groupby(ppdex['concept_name']).agg(['count'])#'mean',
ppdex = ppdex.sort_values(['person_id','condition_start_date'], ascending=True)
ppdex = ppdex.drop_duplicates(subset=['person_id','condition_start_date'], keep='first')

ppdex = pd.DataFrame(ppdex, columns=['person_id','condition_start_date','concept_name'])
ppdex['ppdex'] = 1
ppdex = ppdex.rename({'condition_start_date': 'ppd_dateex'}, axis=1) #'a': 'X', 'b': 'Y'
ppdex = pd.merge(deliverygw3,ppdex,on = ['person_id'], how='inner')
ppdex['diffppdex'] = pd.DataFrame(pd.to_datetime(ppdex['ppd_dateex']) - pd.to_datetime(ppdex['condition_start_date']))
ppdex['diffppdex'] = ppdex['diffppdex'].dt.days

ppdex = ppdex[(ppdex['diffppdex'] >= 0 ) & (ppdex['diffppdex'] <=7)]#7-367days 357
ppdex = ppdex.drop_duplicates(subset=['person_id','condition_start_date'], keep='first')
ppdex[['person_id']].groupby(ppdex['concept_name_y']).agg(['count'])#'mean',
ppdex = pd.DataFrame(ppdex, columns=['person_id','condition_start_date','ppdex','diffppdex'])

deliverygw3 = pd.merge(deliverygw3,ppdex,on = ['person_id','condition_start_date'], how='left')
deliverygw3['ppdex']=deliverygw3['ppdex'].fillna(0)
deliverygw4 = deliverygw3[(deliverygw3['ppdex'] == 0)]
deliverygw4['ppdex'].value_counts()#16966
deliverygw4['ppd'].value_counts()
#0.0    16537
#1.0      429

#exclude antidepressants 0-7days after childbirth

antidepressant['concept_class_id.1'].value_counts()
#the number of 3rd 4th 5th are the same 6043
antidepressant_ex = antidepressant.loc[antidepressant['concept_class_id.1'].isin(['ATC 5th'])]
antidepressant_ex = antidepressant_ex[~antidepressant_ex['concept_code.1'].isin(['N06AA09', 'N06AA04','N06AX21','N05AF01','N06AA10'])]
antidepressant_ex['concept_name.1'].value_counts()  
antidepressant_ex = pd.merge(deliverygw2,antidepressant_ex,on = ['person_id'], how='inner')
#calcualte the time difference between drug_exposure_start_date and delivery_date
antidepressant_ex['diffanti'] = pd.to_datetime(antidepressant_ex['drug_exposure_start_date']) - pd.to_datetime(antidepressant_ex['condition_start_date'])
antidepressant_ex['diffanti'] = antidepressant_ex['diffanti'].dt.days
#select 0-14days
antidepressant_ex = antidepressant_ex[(antidepressant_ex['diffanti'] >= 0 ) & (antidepressant_ex['diffanti'] <= 7)]
antidepressant_ex = antidepressant_ex.drop_duplicates(subset=['person_id','condition_start_date'], keep='first')
antidepressant_ex['ppd'].value_counts()

antidepressant_ex = pd.DataFrame(antidepressant_ex, columns=['person_id','condition_start_date','drug_exposure_start_date','diffanti','ppd'])
antidepressant_ex = antidepressant_ex[(antidepressant_ex['ppd'] == 0 )]
antidepressant_ex = antidepressant_ex.drop('ppd', 1)#0 for rows and 1 for columns
antidepressant_ex['ppdantiex'] = 1

deliverygw4 = pd.merge(deliverygw4,antidepressant_ex,on = ['person_id','condition_start_date'], how='left')
deliverygw4['ppdantiex']=deliverygw4['ppdantiex'].fillna(0)
deliverygw4 = deliverygw4[(deliverygw4['ppdantiex'] == 0)]
deliverygw4['ppdantiex'].value_counts()#16781
deliverygw4['ppd'].value_counts()
#0.0    16469
#1.0      416
...
#condition_list = condition['concept_name_y']
#condition_list = condition_list.drop_duplicates(keep="last")
#condition_list = condition_list.sort_values()
#condition_list = condition_list.to_frame()
#condition_list['variable'] = 'C'
#
#c = list(range(0,4930))
#c = {"variable2" : c}
#c = pd.DataFrame(c)
#condition_list = pd.concat([condition_list,c], axis=0, ignore_index=True)
#condition_list['variable2'] = c
#condition_list = condition_list[(condition_list['variable2'] >= 0 )]
#condition_list['variable2'] = condition_list['variable2'].map(lambda x:str(x))
#condition_list['newvariable'] = condition_list['variable'].str.cat(condition_list['variable2'])
#condition_list = pd.DataFrame(condition_list, columns=['concept_name_y','newvariable'])#4951
...
#rename the new columns for condition name
medication_list = medication2['concept_name.1']
medication_list = medication_list.drop_duplicates(keep="last")
medication_list = medication_list.sort_values()
medication_list = medication_list.to_frame()

#medication_listori = pd.read_csv('/Users/bu/Documents/WCMC/PPD/ppdmodel1907/data/medication_list.csv', delimiter=',', header=0)
#medication_list = pd.merge(medication_list,medication_listori,on=['concept_name.1'],how='left')
#medication_listche = medication_list[(medication_list['newvariable'].isnull())]
#
#
#medication_listche['variable'] = 'M'
#c = list(range(117,136))
#c = {"variable2" : c}
#c = pd.DataFrame(c)
#medication_listche = pd.concat([medication_listche,c], axis=0, ignore_index=True)
#medication_listche['variable2'] = c
#medication_listche = medication_listche[(medication_listche['variable2'] >= 0 )]
#
#medication_listche['variable2'] = medication_listche['variable2'].map(lambda x:str(x))
#medication_listche['newvariable'] = medication_listche['variable'].str.cat(medication_listche['variable2'])
#medication_listche = pd.DataFrame(medication_listche, columns=['concept_name.1','newvariable'])
#
#medication_list = pd.merge(medication_list,medication_listche,on=['concept_name.1'],how='left')
#medication_list.loc[medication_list['newvariable_x'].isnull(),'newvariable_x']=medication_list['newvariable_y']
#medication_list = pd.DataFrame(medication_list, columns=['concept_name.1','newvariable_x'])
...
def trimester(bp):
    if ((bp['gestationday'] - bp['diffbp']) < 84):
        return 1
    if ((bp['gestationday'] - bp['diffbp']) >=84) and ((bp['gestationday'] - bp['diffbp']) < 196):
        return 2
    else:
        return 3
...
###############################################
##insurance
#insurance = pd.read_csv(folder+ "insurance.csv", delimiter=',', header=0)
#insurance = insurance.drop_duplicates(subset=['person_id','condition_start_date','primary_insurance','secondary_insurance','tertiary_insurance'], keep='first')
#insurance = pd.DataFrame(insurance, columns=['person_id','condition_start_date','primary_insurance','secondary_insurance','tertiary_insurance'])
#insurance = pd.merge(deliverygw4,insurance,on = ['person_id'], how='inner')
#insurance['diffin'] = pd.to_datetime(insurance['condition_start_date_x']) - pd.to_datetime(insurance['condition_start_date_y'])
#insurance['diffin'] = insurance['diffin'].dt.days
#insurance = insurance[(insurance['diffin'] == 0 )]
#
#payers = pd.read_csv('/Users/bu/Documents/WCMC/PPD/ppdmodel1907/data/modelfile/orifile/wcm/payers.csv', delimiter=',', header=0)
#insurance = pd.merge(insurance,payers,on = ['primary_insurance'], how='left')
#payers = payers.rename({'primary_insurance': 'secondary_insurance','payer1': 'payer2'}, axis=1) #'a': 'X', 'b': 'Y'
#insurance = pd.merge(insurance,payers,on = ['secondary_insurance'], how='left')
#payers = payers.rename({'secondary_insurance': 'tertiary_insurance','payer2': 'payer3'}, axis=1) #'a': 'X', 'b': 'Y'
#insurance = pd.merge(insurance,payers,on = ['tertiary_insurance'], how='left')
#
#def insur(insurance):
#    if (insurance['payer1'] == 'COMMERCIAL' or insurance['payer2'] == 'COMMERCIAL' or insurance['payer3'] == 'COMMERCIAL'):
#        return 1
#insurance['COMMERCIAL'] = insurance.apply(insur, axis=1)
#
#def insur(insurance):
#    if (insurance['payer1'] == 'MEDICARE' or insurance['payer2'] == 'MEDICARE' or insurance['payer3'] == 'MEDICARE'):
#        return 1
#insurance['MEDICARE'] = insurance.apply(insur, axis=1)
#
#def insur(insurance):
#    if (insurance['payer1'] == 'NEED_BASED PUBLIC' or insurance['payer2'] == 'NEED_BASED PUBLIC' or insurance['payer3'] == 'NEED_BASED PUBLIC'):
#        return 1
#insurance['NEED_BASED PUBLIC'] = insurance.apply(insur, axis=1)
#
#def insur(insurance):
#    if (insurance['payer1'] == 'COMMERCIAL/EMPLOYER' or insurance['payer2'] == 'COMMERCIAL/EMPLOYER' or insurance['payer3'] == 'COMMERCIAL/EMPLOYER'):
#        return 1
#insurance['COMMERCIALEMPLOYER'] = insurance.apply(insur, axis=1)
#
#def insur(insurance):
#    if (insurance['payer1'] == 'MEDICAID' or insurance['payer2'] == 'MEDICAID' or insurance['payer3'] == 'MEDICAID'):
#        return 1
#insurance['MEDICAID'] = insurance.apply(insur, axis=1)
#
#def insur(insurance):
#    if (insurance['payer1'] == 'Student health plan' or insurance['payer2'] == 'Student health plan' or insurance['payer3'] == 'Student health plan'):
#        return 1
#insurance['Studenthealthplan'] = insurance.apply(insur, axis=1)
#
#def insur(insurance):
#    if (insurance['payer1'] == 'INTERNATIONAL' or insurance['payer2'] == 'INTERNATIONAL' or insurance['payer3'] == 'INTERNATIONAL'):
#        return 1
#insurance['INTERNATIONAL'] = insurance.apply(insur, axis=1)
#
#insurance = pd.DataFrame(insurance, columns=['person_id','condition_start_date_x','COMMERCIAL','MEDICARE','NEED_BASED PUBLIC','COMMERCIALEMPLOYER','MEDICAID','Studenthealthplan','INTERNATIONAL'])
#insurance = insurance.rename({'condition_start_date_x': 'condition_start_date'}, axis=1)
#insurance = insurance.fillna(0)
#insurance = insurance.drop_duplicates()
#
#insurance = insurance.sort_values(['person_id','condition_start_date','COMMERCIAL','MEDICARE','NEED_BASED PUBLIC','COMMERCIALEMPLOYER','MEDICAID'], ascending=False)
#insurance = insurance.drop_duplicates(subset=['person_id','condition_start_date'], keep='first')
#
##############################################
#amniocentesis
...
#medication_list.to_csv('/Users/bu/Documents/WCMC/PPD/ppdmodel1907/data/medication_listnew.csv', sep=',',index=0)
#condition_list.to_csv('/Users/bu/Documents/WCMC/PPD/ppdmodel1907/data/condition_listnew.csv', sep=',',index=0)
...
#anxiety = pd.read_csv('/Users/shuojiawang/Documents/ppdmodel/orifile/anxiety.txt', delimiter=',', header=0)
#mooddisorder = pd.read_csv('/Users/shuojiawang/Documents/ppdmodel/orifile/mooddisorder.txt', delimiter=',', header=0)
#organicdisorders = pd.read_csv('/Users/shuojiawang/Documents/ppdmodel/orifile/organicdisorders.txt', delimiter=',', header=0)
#otherdisorder = pd.read_csv('/Users/shuojiawang/Documents/ppdmodel/orifile/otherdisorder.txt', delimiter=',', header=0)
#personalitydisorder = pd.read_csv('/Users/shuojiawang/Documents/ppdmodel/orifile/personalitydisorder.txt', delimiter=',', header=0)
#schizophrenic = pd.read_csv('/Users/shuojiawang/Documents/ppdmodel/orifile/schizophrenic.txt', delimiter=',', header=0)
#substance = pd.read_csv('/Users/shuojiawang/Documents/ppdmodel/orifile/substance.txt', delimiter=',', header=0)
#check the concept_name
#anxiety_name = anxiety.groupby(by=['concept_id.1','concept_name.1','condition_source_value'])
...
wcmmodelhistory.to_csv('/Users/bu/Documents/WCMC/PPD/ppdmodel1907/data/modelfile/wcm/wcmmodelwithhistory.csv', sep=',',index=0)
...
#table1 file
table1 = pd.merge(demo,marital,on = ['person_id','condition_start_date'], how='left')
table1['marital']=table1['marital'].fillna(1)
table1 = pd.merge(table1,visit,on = ['person_id','condition_start_date'], how='left')
table1['edvisitcount']=table1['edvisitcount'].fillna(0)
table1 = pd.merge(table1,amniocentesis,on = ['person_id','condition_start_date'], how='left')
table1['amniocentesis']=table1['amniocentesis'].fillna(0)

#table1 = pd.merge(table1,insurance,on = ['person_id','condition_start_date'], how='left')
table1=table1.fillna(0)
table1 = pd.merge(table1,weight,on = ['person_id','condition_start_date'], how='left')
table1 = pd.merge(table1,bp,on = ['person_id','condition_start_date'], how='left')
table1 = pd.merge(table1,bmi,on = ['person_id','condition_start_date'], how='left')
table1 = table1.fillna(table1.mean())
table1 = pd.merge(table1,history,on = ['person_id','condition_start_date'], how='left')
table1.to_csv('/Users/bu/Documents/WCMC/PPD/ppdmodel1907/data/modelfile/wcm/wcmnewtable1.csv', sep=',',index=0)



###verify data
che = pd.read_csv('/Users/bu/Documents/WCMC/PPD/ppdmodel1907/data/modelfile/wcm/oldversion/wcmmodelwithhistory2w.csv', delimiter=',', header=0)
che = che[che['ppd']>0]
che = pd.DataFrame(che,columns=['person_id','condition_start_date','ppd'])

che2 = pd.DataFrame(wcmmodelhistory,columns=['person_id','condition_start_date','ppd'])
che2 = che2[che2['ppd']>0]
che2 = pd.merge(che2,che,on = ['person_id','condition_start_date'], how='right')

chee = antidepressant[antidepressant['mrn']==78632956]
