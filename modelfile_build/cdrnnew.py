...
# -*- coding: utf-8 -*-
"""
Created on Mon Jul  1 13:19:26 2019
...
ppd = ppd[(ppd['diffppd'] > 7 ) & (ppd['diffppd'] < 367)]#0-367day 360;7-367days 189
ppd = ppd.drop_duplicates(subset=['person_id','condition_start_date'], keep='first')
...
##############################################
ppdex = pd.read_csv('/Users/bu/Documents/WCMC/PPD/ppdmodel1907/data/modelfile/orifile/cdrn/ppd.csv', delimiter=',', header=0)
...
antidepressant['concept_class_id'].value_counts()
#the number of 3rd 4th 5th are the same 33958
antidepressant_ex = antidepressant.loc[antidepressant['concept_class_id'].isin(['ATC 5th'])]
antidepressant_ex = antidepressant_ex[~antidepressant_ex['concept_code'].isin(['N06AA09', 'N06AA04','N06AX21','N05AF01','N06AA10'])]
antidepressant_ex['concept_name'].value_counts()  
...
condition = condition[(condition['diffcondition'] >= 0 ) & (condition['diffcondition'] <= 280)]
condition = condition.drop_duplicates(subset=['person_id','condition_start_date','concept_name'], keep='first')
...
medication2 = medication2[(medication2['diffmedication'] >= 0 ) & (medication2['diffmedication'] <= 280)]
medication2 = medication2.drop_duplicates(subset=['person_id','drug_exposure_start_date_y','concept_name'], keep='first')
medication2 = pd.DataFrame(medication2, columns=['person_id','drug_exposure_start_date_y','condition_start_date','concept_name','diffmedication'])
...
visit = visit[(visit['diffvisit'] >= 0 ) & (visit['diffvisit'] <= 280)]
...
cdrnmodelhistory.to_csv('/Users/bu/Documents/WCMC/PPD/ppdmodel1907/data/modelfile/cdrn/cdrnnew.csv', sep=',',index=0)
...
demo = pd.merge(demo,visit,on = ['person_id','condition_start_date'], how='left')
demo['edvisitcount']=demo['edvisitcount'].fillna(0)
demo = pd.merge(demo,amniocentesis,on = ['person_id','condition_start_date'], how='left')
demo['amniocentesis']=demo['amniocentesis'].fillna(0)
demo = pd.merge(demo,weight,on = ['person_id','condition_start_date'], how='left')
demo = pd.merge(demo,bp,on = ['person_id','condition_start_date'], how='left')
demo = pd.merge(demo,bmi,on = ['person_id','condition_start_date'], how='left')
demo = demo.fillna(demo.mean())
demo.to_csv('/Users/bu/Desktop/cdrndemo.csv', sep=',',index=0)
...
#table1 = pd.merge(demo,visit,on = ['person_id','condition_start_date'], how='left')
#table1['edvisitcount']=table1['edvisitcount_y'].fillna(0)
...
table1 = pd.merge(demo,history,on = ['person_id','condition_start_date'], how='left')
table1.to_csv('/Users/bu/Documents/WCMC/PPD/ppdmodel1907/data/modelfile/wcm/cdrnnewtable1.csv', sep=',',index=0)
